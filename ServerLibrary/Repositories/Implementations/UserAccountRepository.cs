...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
    {
        ...
        public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
        {
            ...
            if (user is null)
                return new LoginResponse(false, "User token could not be generated becuase user not found");
            ...
            var updateRegreshToken = await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(_ => _.UserId == user.Id);
            if( updateRegreshToken is null)
                return new LoginResponse(false, "Refresh token could not be generated becuase user has not sign in");

            updateRegreshToken.Token = refreshToken;
            await appDbContext.SaveChangesAsync();
